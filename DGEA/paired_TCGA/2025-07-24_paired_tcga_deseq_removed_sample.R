...
count_data <- read.csv("C:/Users/leahe/Documents/LIFE4137_independent_project/xena/paired_TCGA/removed_sample/STN_cancer_pairs_rawcounts.csv", row.names = 1, check.names = FALSE)
condition <- read.csv("C:/Users/leahe/Documents/LIFE4137_independent_project/xena/paired_TCGA/removed_sample/paired_TCGA_col_data.csv", header = TRUE, row.names = 1)
...
res <- results(dds, name="condition_HCC_vs_Solid_tissue_normal") # y (HCC) vs x control (STN)
res
...
# padj < 0.01
sum(res$padj < 0.01, na.rm=TRUE) # 587
# padj < 0.05
sum(res$padj < 0.05, na.rm=TRUE) # 884
...
with(res[topGene, ], {
  ...
  text(baseMean, log2FoldChange, topGene, pos=1, col="limegreen")
})


# Plot to compare log counts of the most significant gene (smallest padj value)
clean_d <- plotCounts(dds, gene=which.min(res$padj), intgroup=c("condition", "sample"), 
                      returnData=TRUE)
# Coloured points based on sample
ggplot(clean_d, aes(x=condition, y=count, colour=sample)) + 
...
# Top differential expressed gene
which.min(res$padj) # 20167 - ENSG00000205358.4 gene
# log count statistics of the top differential expressed gene
STN_gene_count <- clean_d %>%
  filter(condition=="Solid_tissue_normal")
mean(STN_gene_count[["count"]]) # 8.965703
median(STN_gene_count[["count"]])  # 9.625185

HCC_gene_count <- clean_d %>%
  filter(condition=="HCC")
mean(HCC_gene_count[["count"]]) # 1.744215
median(HCC_gene_count[["count"]]) # 1.399659
...
colnames(sampleDistMatrix) <- paste(vsd$condition, vsd$sample, sep = " - " )
...
# Now make a label column that labels the top_genes
# Create a new column "label" that will contain the name of the top 10 differentially expressed genes (NA in case they are not)
volcano$label <- ifelse(volcano$Gene %in% top_genes, volcano$Gene, NA)

# Convert geneID label to gene symbol
volcano$label[volcano$label=="ENSG00000175063.17"]<-"UBE2C"
volcano$label[volcano$label=="ENSG00000164283.13"]<-"ESM1"
volcano$label[volcano$label=="ENSG00000147257.15"]<-"GPC3"
volcano$label[volcano$label=="ENSG00000204291.11"]<-"COL15A1"
volcano$label[volcano$label=="ENSG00000117399.14"]<-"CDC20"
volcano$label[volcano$label=="ENSG00000205358.4"]<-"MT1H"
volcano$label[volcano$label=="ENSG00000125144.14"]<-"MT1G"
volcano$label[volcano$label=="ENSG00000019169.10"]<-"MARCO"
volcano$label[volcano$label=="ENSG00000160339.16"]<-"FCN2"
volcano$label[volcano$label=="ENSG00000198417.7"]<-"MT1F"
...
ggplot(data=volcano, aes(x=log2FoldChange, y=-log10(padj), col=diffexpressed, label=label)) +
  ...
  ylim(-5, 25) +
  xlim(-5, 5) +
  geom_text_repel(max.overlaps = 20, size = 5, col = "black", force=0)
...
fit
write.csv(as.data.frame(upreg_GO),file="C:/Users/leahe/Documents/LIFE4137_independent_project/xena/removed_versions/GO_analysis/upreg/paired_upreg.csv")
...
fit
write.csv(as.data.frame(downreg_GO),file="C:/Users/leahe/Documents/LIFE4137_independent_project/xena/removed_versions/GO_analysis/downreg/paired_downreg.csv")
...
write.table(upreg_05["Gene"],
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/paired_TCGA/removed_sample/upreg_05_genes.txt",
            sep = "",
            row.names = FALSE,
            col.names = FALSE
)
...
write.table(upreg_10_sum,
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/paired_TCGA/removed_sample/upreg_summary.txt",
            sep = ",",
            row.names = FALSE,
            col.names = FALSE
)
...
write.table(downreg_05["Gene"],
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/paired_TCGA/removed_sample/downreg_05_genes.txt",
            sep = "",
            row.names = FALSE,
            col.names = FALSE
)
...
write.table(downreg_10_sum,
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/paired_TCGA/removed_sample/downreg_summary.txt",
            sep = ",",
            row.names = FALSE,
            col.names = FALSE
)
...
write.table(top_genes_data,
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/paired_TCGA/removed_sample/all_top5_dgea_genes.txt",
            sep = ",",
            row.names = FALSE,
            col.names = TRUE
)
