...
count_data <- read.csv("C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s12_vs_s34/all_stages_rawcounts.csv", row.names = 1, check.names = FALSE) # Use same file path as s12_s34 since samples are unchanged
condition <- read.csv("C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1_vs_s234/s1vs234_raw_col_data.csv", header = TRUE, row.names = 1)
...
res <- results(dds, name="condition_Stage1_HCC_vs_Advanced_stage_HCC") # y (stage1) vs x control (advanced stage)
...
# pval < 0.01
sum(res$padj < 0.01, na.rm=TRUE) # 25
# pval < 0.05
sum(res$padj < 0.05, na.rm=TRUE) # 217
...
with(res[topGene, ], {
  ...
  text(baseMean+23, 0.5, topGene, pos=2, col="limegreen", cex = 0.8)
})
...
which.min(res$padj) # 1176 - ENSG00000072080.11 gene
# log count statistics of the top differentially expressed gene
advanced_gene_count <- clean_d %>%
  filter(condition=="Advanced_stage_HCC")
mean(advanced_gene_count[["count"]]) # 9.580037
median(advanced_gene_count[["count"]])  # 10.3666

stage1_gene_count <- clean_d %>%
  filter(condition=="Stage1_HCC")
mean(stage1_gene_count[["count"]]) # 11.649
median(stage1_gene_count[["count"]]) # 12.11544
...
ggplot(data=volcano, aes(x=log2FoldChange, y=-log10(padj), col=diffexpressed, label=label)) +
  ...
  ylim(-0.5, 4) +
  xlim(-2, 2) +
  geom_text_repel(max.overlaps = 20, size = 3, col = "black", force = 20)
...
# GO ontology
# Upregulated pathways - Not working?
...
fit
write.csv(as.data.frame(upreg_GO),file="C:/Users/leahe/Documents/LIFE4137_independent_project/xena/removed_versions/GO_analysis/upreg/advanced_s1_upreg.csv")
...
fit
write.csv(as.data.frame(downreg_GO),file="C:/Users/leahe/Documents/LIFE4137_independent_project/xena/removed_versions/GO_analysis/downreg/advanced_s1_downreg.csv")



# Upregulated significant genes in volcano plot
sig_upreg_genes <- sig_upreg_genes %>%
  arrange(desc(-padj))
write.table(sig_upreg_genes,
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1_vs_s234/upreg_genes.txt",
            sep = ",",
            row.names = FALSE,
            col.names = FALSE
)
...
# Top 10 significant genes (padj < 0.05) (Include all downreg genes but no upreg genes)
sig_10 <- volcano %>%
  arrange(desc(-padj))
sig_10 <- sig_10[1:10, ]
write.table(sig_10,
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1_vs_s234/sig_10_genes.txt",
            sep = ",",
            row.names = FALSE,
            col.names = FALSE
)
...
write.table(downreg_05["Gene"],
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/s1_vs_s234/hs3_downreg_05_genes.txt",
            sep = "",
            row.names = FALSE,
            col.names = FALSE
)
...
write.table(downreg_50["Gene"],
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1_vs_s234/s1_s234_downreg_50_genes.txt",
            sep = "",
            row.names = FALSE,
            col.names = FALSE
)
...
downreg_05 <- downreg %>%
  arrange(desc(log2FoldChange))
write.table(downreg_05["Gene"],
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1_vs_s234/s1_s234_downreg_05_genes.txt",
            sep = "",
            row.names = FALSE,
            col.names = FALSE
)
...
write.table(top_genes_data,
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1_vs_s234/all_top5_dgea_genes.txt",
            sep = ",",
            row.names = FALSE,
            col.names = TRUE
)
