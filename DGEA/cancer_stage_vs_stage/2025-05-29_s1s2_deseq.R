...
count_data <- read.csv("C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1s2/stage1_stage2_rawcounts.csv", row.names = 1, check.names = FALSE)
condition <- read.csv("C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1s2/s1s2_raw_col_data.csv", header = TRUE, row.names = 1)
...
res <- results(dds, name="condition_Stage2_HCC_vs_Stage1_HCC") # y experiment (stage2) against x control (stage1)
...
# How many adjusted p-values were less than 0.05 FOR ALL SAMPLES?
sum(res$padj < 0.05, na.rm=TRUE) # 2
...
ggplot(data=volcano, aes(x=log2FoldChange, y=-log10(padj), col=diffexpressed, label=label)) +
  ...
  scale_color_manual(values=c("grey", "firebrick2"),
                     labels=c("No", "Upregulated")) +
  geom_vline(xintercept=c(-0.6, 0.6), col="grey25") +
  geom_hline(yintercept=-log10(0.05), col="grey25") +
  ylim(-5, 5) +
  xlim(-5, 5) +
  geom_text_repel(max.overlaps = 30, size = 3, force = 35, col = "black")

# Density plot
ggplot(volcano, aes(x=log2FoldChange, fill=diffexpressed)) +
  geom_density(alpha=0.6) +
  scale_fill_manual(values = c("grey", "red"), name="Differential Gene Expression") +
  theme_classic() +
...
fit
write.csv(as.data.frame(upreg_GO),file="C:/Users/leahe/Documents/LIFE4137_independent_project/xena/removed_versions/GO_analysis/upreg/s1s2_upreg.csv")

# Upregulated, genes where padj < 0.05
upreg_05 <- upreg %>%
  arrange(desc(log2FoldChange))
write.table(upreg_05["Gene"],
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1s2/s1s2_upreg_05_genes.txt",
            sep = "",
            row.names = FALSE,
            col.names = FALSE
)
...
write.table(upreg_sum,
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1s2/s1s2_upreg_summary.txt",
            sep = ",",
            row.names = FALSE,
            col.names = FALSE
)
...
write.table(top_genes_data,
            file = "C:/Users/leahe/Documents/LIFE4137_independent_project/xena/clean_script/s1s2/all_top5_dgea_genes.txt",
            sep = ",",
            row.names = FALSE,
            col.names = TRUE
)
